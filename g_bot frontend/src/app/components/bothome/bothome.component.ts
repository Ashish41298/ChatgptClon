...
@Component({
  selector: 'app-bothome',
  templateUrl: './bothome.component.html',
  styleUrls: ['./bothome.component.css'],
...
export class BothomeComponent implements OnInit {
  ...
  constructor(private elementref: ElementRef, private router: Router, private renderer: Renderer2, private share:SharedataService) {
    ...
    if (localStorage.getItem('chat')) {
      const dd = JSON.parse(localStorage.getItem('chat')!);
      this.bacChat = dd;
    } else { '' };
  ...
  handlesubmit = async (e: any) => {
    ...
    fetch(`${mopnnodekey}/chat`, { //"http://localhost:3000/api/chat"
      method: 'POST',
      headers: {
      ...
      const content = data.bot.content;
      if (content) {
